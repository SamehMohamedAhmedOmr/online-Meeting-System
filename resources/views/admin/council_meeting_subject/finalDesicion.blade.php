...
<div class="main-panel">
    <div class="alert alert-success" role="alert" hidden id="succes"
        style="font-weight: bold; text-align: center; color: #fff;">
        {{__("home.Saved")}}
    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group {{ $errors->has('final_decision') ? 'has-error' : ''}}"
                            ...
                            <label for="membershipRadios1"
                                class="control-label d-block {{ (App::getLocale() == 'ar') ? 'text-right' : 'text-left' }}">{{ __('Staff.Finaldecision') }}
                                <span style="color:red !important;">*</span></label>
                            <div class="row"
                                style="direction: ltr; flex-wrap: nowrap; text-align: center; {{ (App::getLocale() == 'ar') ? 'flex-direction: row-reverse;' : '' }} ">
                                <div
                                    class="col-sm-6 {{ (App::getLocale() == 'ar')?'mr-sm-3 ml-sm-5':'ml-sm-3 mr-sm-5' }}">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group {{ $errors->has('person_redirected') ? 'has-error' : ''}}">
                                    <label for="person_redirected"
                                        class="control-label d-block {{ (App::getLocale() == 'ar') ? 'text-right' : 'text-left' }}">{{ __('Staff.Personredirected') }}
                                        <span style="color:red !important;">*</span></label>

                                    <select class="form-control specialSelect" name="person_redirected"
                                        id='person_redirected' required>
                                        <option selected hidden value="">
                                            {{ __('placeholder.Select person to redirect') }}</option>
                            ...
                            <div class="col-md-6">
                                <div
                                    class="form-group {{ $errors->has('next_council_definition_id') ? 'has-error' : ''}}">
                                    <label for="next_council_definition_id"
                                        class="control-label d-block {{ (App::getLocale() == 'ar') ? 'text-right' : 'text-left' }}">{{ __('Staff.Nextcouncildefinition') }}
                                        <span style="color:red !important;">*</span></label>
                                    <select class="form-control specialSelect" name="next_council_definition_id"
                                        ...
                                        @endif
                                        @else
                                        @if (old('next_council_definition_id') != null &&
                                        old('next_council_definition_id') == $council->id)
                                        <option value="{{ $council->id}}" selected>
                                            {{ $council->council_name }}
                                        </option>
                                        @else
                        ...
                        <div class="form-group {{ $errors->has('final_decision_description') ? 'has-error' : ''}}">
                            <label for="final_decision_description"
                                class="control-label d-block {{ (App::getLocale() == 'ar') ? 'text-right' : 'text-left' }}">{{ __('Staff.Final Decision Description') }}
                                <span style="color:red !important;">*</span></label>
